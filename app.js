...
const STARTER = [
  ...
  name, slug, category, icon, status: 'Frontend live', color: PALETTE[i % PALETTE.length],
  frontend_url: V(slug), github_repo: `xaglobally-lgtm/${slug}`, vercel_project: slug,
...
const actions = {
  ...
  seed: async () => { const { error } = await sb.from('planet_apps').insert(STARTER); if (error) return fail(error); await loadAll(); render(); toast('14 apps added'); },
  export: () => {
